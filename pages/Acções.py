...
if 'df_carregado' in st.session_state:
    ...
    accoes = df[df['Tipologia do Título'] =='Acções']
    
    accoes['Variação (%)'] = limpar_e_converter(accoes['Variação (%)'])
    accoes['Quantidade'] = limpar_e_converter(accoes['Quantidade'])
    accoes['Montante'] = limpar_e_converter(accoes['Montante'])
    accoes['N° de Negócios'] = limpar_e_converter(accoes['N° de Negócios'])
  
    col1,col2, =st.columns(2)
    with col1: 

        fig5=px.bar(accoes,x="Montante",y="Valor Mobiliário",text="Montante",color="Valor Mobiliário",title=" Montante Movimentado por Titulo",orientation="h",color_discrete_sequence=['rgba(34, 197, 94, 0.4)','#3C3C3D','#007AFF','#E82127'])
        st.plotly_chart(fig5) 
        maior_quantidade = accoes["Quantidade"].max()
        nome_maior_quantidade=accoes[accoes["Quantidade"] >= maior_quantidade]
        ...
        <div style='
            ...
            margin-top:500px;'
        >
        <h3 style='margin-bottom:;color:rgba(34, 197, 94, 0.4);'>Quantidade Total Transacionada por Título</h3>
        ...
        fig = px.bar(accoes,x='Valor Mobiliário',y='Variação (%)', title='Análise de Variação de Preço',color='Valor Mobiliário',color_discrete_sequence=['rgba(34, 197, 94, 0.4)','#3C3C3D','#007AFF','#E82127'])
        st.plotly_chart(fig)

        maior_volume = accoes["N° de Negócios"].max()
        nome_maior_volume =accoes[accoes["N° de Negócios"] == maior_volume]
        ...
        <div style='
            ...
            font-family: 'Noto Sans', sans-serif;'
        >
        <h3 style='margin-bottom:;color:rgba(34, 197, 94, 0.4);'> Volume de Negócios por Título </h3>
    ...
    with col2:
        maior_montante = accoes['Montante'].max()
        nome_maior_montante = accoes[accoes['Montante'] == maior_montante]
       ...
       <p>Este gráfico mostra <strong> quanto foi movimentado em dinheiro por cada título, </strong> somando compras e vendas.  </p> 
        ...
        cores=['rgba(34, 197, 94, 0.4)','#3C3C3D','#007AFF','#E82127']
        fig2 = px.pie(accoes,names='Valor Mobiliário', values='Quantidade',hole=0.7,title='Quantidade Total Transacionada por Título',color_discrete_sequence=cores)  
        st.plotly_chart(fig2)

        variacao_max =accoes[accoes['Variação (%)'] == accoes['Variação (%)'].max()]
        variacao_min =accoes[accoes['Variação (%)'] == accoes['Variação (%)'].min()]
        ...
        <div style='
            ...
            font-family: 'Noto Sans', sans-serif;'
        >
        <h3 style='margin-bottom:;color:rgba(34, 197, 94, 0.4);'>Análise de Variação de Preço </h3>
        ...
        fig5=px.bar(accoes,x="N° de Negócios",y="Valor Mobiliário",text="N° de Negócios",color="Valor Mobiliário",title=" Volume de Negócios por Título",orientation="h",color_discrete_sequence=['rgba(34, 197, 94, 0.4)','#3C3C3D','#007AFF','#E82127'])
        st.plotly_chart(fig5)
        st.divider()
 
 
    comprar = accoes[
        (accoes["N° de Negócios"] >= accoes["N° de Negócios"].max())&    
        (accoes["Quantidade"] >= accoes["Quantidade"].max())&
        (accoes["Montante"] >= accoes["Montante"].max())]  
    ler_comprar = comprar["Valor Mobiliário"].tolist()
    ver4 = (','.join(ler_comprar))
    mensangem =f"""
        
        <h2 style='margin-bottom:;color:rgba(34, 197, 94, 0.4);'>Destaque </h2>
    ...
    mensangem =f"""
        <div style='
            ...
            font-family: 'Noto Sans', sans-serif;'
        >
        <h3 style='margin-bottom:;color:rgba(34, 197, 94, 0.4);'> {ver4} </h3>

        <p><strong>Montande</strong>:<strong style='color:rgba(34, 197, 94, 0.4);'>{maior_montante} kz</strong>  .O {ver4} tem a maior um montante movimentado supeior a media </p>
        <p><strong>Quantidade</strong>:</strong><strong  style='color:rgba(34, 197, 94, 0.4);'>{maior_quantidade} </strong>  .O {ver4} tem a maior uma quantia transacionada  supeior a media</p>
        <p><strong>Numero de Negocios</strong>:</strong><strong  style='color:rgba(34, 197, 94, 0.4);'>{maior_volume:,.0f} </strong>  .O {ver4} tem a maior o volume de Negocios supeior a media</p>
        </p> 
        </div> 
